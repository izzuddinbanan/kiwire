...
if (in_array("Device -> Zone", $kiw_roles) == false) {
...
if ($kiw_request['method'] == "GET") {
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_data = $kiw_db->query_first("SELECT id, tenant_id, updated_date, master_id, nasid, ipaddr, vlan, ssid, dzone, priority FROM kiwire_zone_child {$kiw_tenant_query} LIMIT 1");
        ...
        $kiw_data = $kiw_db->fetch_array("SELECT id, tenant_id, updated_date, master_id, nasid, ipaddr, vlan, ssid, dzone, priority FROM kiwire_zone_child {$kiw_tenant_query} ORDER BY {$kiw_config['column']} {$kiw_config['order']} LIMIT {$kiw_config['offset']}, {$kiw_config['limit']}");
    ...
    foreach (array("tenant_id", "master_id") as $kiw_key) {


        if (!isset($_REQUEST[$kiw_key])) {
    ...
    $kiw_id = $kiw_db->query_first("SELECT COUNT(*) AS kcount FROM kiwire_zone WHERE name = '{$kiw_data['master_id']}' AND tenant_id = '{$kiw_data['tenant_id']}' LIMIT 1");


    if ($kiw_id['kcount'] > 0) {


        $kiw_db->insert("kiwire_zone_child", $kiw_data);

        if ($kiw_db->db_affected_row > 0) {

            echo json_encode(array("status" => "success", "message" => "", "data" => ""));

            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} created zone rule {$kiw_data['master_id']}");
         ...
         echo json_encode(array("status" => "success", "message" => "No master_id existed in Zone table", "data" => ""));
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_db->update("kiwire_zone_child", $kiw_data, "id = '{$kiw_request['id']}' AND tenant_id = '{$kiw_data['tenant_id']}'");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} updated zone rule {$kiw_request['master_id']}");
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_name = $kiw_db->query_first("SELECT master_id FROM kiwire_zone_child {$kiw_tenant_query} LIMIT 1");

        $kiw_db->query("DELETE FROM kiwire_zone_child {$kiw_tenant_query} LIMIT 1");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} deleted zone rule {$kiw_name['master_id']}");
