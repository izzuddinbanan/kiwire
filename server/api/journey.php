...
if (in_array("Login Engine -> Journey", $kiw_roles) == false) {
...
if ($kiw_request['method'] == "GET") {
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_data = $kiw_db->query_first("SELECT * FROM kiwire_login_journey {$kiw_tenant_query} LIMIT 1");
        ...
        $kiw_data = $kiw_db->fetch_array("SELECT * FROM kiwire_login_journey {$kiw_tenant_query} ORDER BY {$kiw_config['column']} {$kiw_config['order']} LIMIT {$kiw_config['offset']}, {$kiw_config['limit']}");
    ...
    foreach (array("tenant_id", "page_list", "journey_name", "status") as $kiw_key) {
    ...
    $kiw_test = $kiw_db->query_first("SELECT COUNT(*) AS kcount FROM kiwire_login_journey WHERE name = '{$kiw_data['journey_name']}' AND tenant_id = '{$kiw_data['tenant_id']}'");


    if ($kiw_test['kcount'] == 0) {


        $kiw_db->insert("kiwire_login_journey", $kiw_data);

        if ($kiw_db->db_affected_row > 0) {

            echo json_encode(array("status" => "success", "message" => "", "data" => ""));

            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} created journey {$kiw_data['journey_name']}");
         ...
         echo json_encode(array("status" => "success", "message" => "Journey name already existed", "data" => ""));
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_db->update("kiwire_login_journey", $kiw_data, "id = '{$kiw_request['id']}' AND tenant_id = '{$kiw_data['tenant_id']}'");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} updated journey {$kiw_request['journey_name']}");
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_name = $kiw_db->query_first("SELECT journey_name FROM kiwire_login_journey {$kiw_tenant_query} LIMIT 1");
 
        $kiw_db->query("DELETE FROM kiwire_login_journey {$kiw_tenant_query} LIMIT 1");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} deleted journey {$kiw_name['journey_name']}");
