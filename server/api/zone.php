...
if ($kiw_request['method'] == "GET") {
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_data = $kiw_db->query_first("SELECT * FROM kiwire_zone {$kiw_tenant_query} LIMIT 1");
        ...
        $kiw_data = $kiw_db->fetch_array("SELECT * FROM kiwire_zone {$kiw_tenant_query} ORDER BY {$kiw_config['column']} {$kiw_config['order']} LIMIT {$kiw_config['offset']}, {$kiw_config['limit']}");
    ...
    foreach (array("tenant_id", "name", "status") as $kiw_key) {
    ...
    if (isset($_REQUEST['priority'])){
        ...
        $kiw_data['priority'] = $_REQUEST['priority'];

    } else $kiw_data['priority'] = 0;
    ...
    foreach ($_REQUEST as $kiw_key => $kiw_value){
        ...
        if (!isset($kiw_data[$kiw_key]) && !in_array($kiw_key, array("id", "updated_date", "created_by"))){
    ...
    $kiw_data['created_by'] = "API";


    // check if the name already been used

    $kiw_test = $kiw_db->query_first("SELECT COUNT(*) AS kcount FROM kiwire_zone WHERE name = '{$kiw_data['name']}' AND tenant_id = '{$kiw_data['tenant_id']}'");


    if ($kiw_test['kcount'] == 0) {


        $kiw_db->insert("kiwire_zone", $kiw_data);

        if ($kiw_db->db_affected_row > 0) {

            echo json_encode(array("status" => "success", "message" => "", "data" => ""));

            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} created zone {$kiw_data['name']}");
    ...
    foreach ($_REQUEST as $kiw_key => $kiw_value){
        ...
        if (!isset($kiw_data[$kiw_key]) && !in_array($kiw_key, array("id", "updated_date", "name", "created_by"))){
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_db->update("kiwire_zone", $kiw_data, "id = '{$kiw_request['id']}' AND tenant_id = '{$kiw_data['tenant_id']}'");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} updated zone {$kiw_request['name']}");
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_name = $kiw_db->query_first("SELECT name FROM kiwire_zone {$kiw_tenant_query} LIMIT 1");


        $kiw_db->query("DELETE FROM kiwire_zone {$kiw_tenant_query} LIMIT 1");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} deleted zone {$kiw_name['name']}");
