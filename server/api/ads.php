...
if (in_array("Campaign -> Ads Management", $kiw_roles) == false) {
...
if ($kiw_request['method'] == "GET") {
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_data = $kiw_db->query_first("SELECT * FROM kiwire_campaign_ads {$kiw_tenant_query} LIMIT 1");
        ...
        $kiw_data = $kiw_db->fetch_array("SELECT * FROM kiwire_campaign_ads {$kiw_tenant_query} ORDER BY {$kiw_config['column']} {$kiw_config['order']} LIMIT {$kiw_config['offset']}, {$kiw_config['limit']}");
    ...
    foreach (array("tenant_id", "name", "msg", "type", "link", "viewport") as $kiw_key) {
    ...
    $kiw_test = $kiw_db->query_first("SELECT COUNT(*) AS kcount FROM kiwire_campaign_ads WHERE name = '{$kiw_data['name']}' AND tenant_id = '{$kiw_data['tenant_id']}'");


    if ($kiw_test['kcount'] == 0) {


        $kiw_db->insert("kiwire_campaign_ads", $kiw_data);
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} created ads {$kiw_data['name']}");
    ...
    $_REQUEST = json_decode($_REQUEST, true);
     ...
     foreach (array("tenant_id") as $kiw_key) {
        ...
        if (empty($_REQUEST[$kiw_key])) {
            ...
            die(json_encode(array("status" => "success", "message" => "Missing required data [ {$kiw_key} ]", "data" => "")));
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_db->update("kiwire_campaign_ads", $kiw_data, "id = '{$kiw_request['id']}' AND tenant_id = '{$kiw_data['tenant_id']}'");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} updated ads {$kiw_request['name']}");
    ...
    if (count($request_module) == 2) {
        ...
        $kiw_name = $kiw_db->query_first("SELECT name FROM kiwire_campaign_ads {$kiw_tenant_query} LIMIT 1");
 
        $kiw_db->query("DELETE FROM kiwire_campaign_ads {$kiw_tenant_query} LIMIT 1");
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} deleted ads {$kiw_name['name']}");
