...
if ($kiw_request['method'] == "GET") {
    ...
    if ($kiw_creation['status'] == "success") {
        ...
        //     logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} created {$kiw_data['quantity']} voucher with bulk id {$kiw_creation['voucher']}");
        ...
        // } else {
            ...
            logger_api($kiw_api['tenant_id'], "{$kiw_request['api_key']} created {$kiw_data['quantity']} voucher {$kiw_data['voucher']}");
    ...
    if (count($request_module) == 2) {
        ...
        if ($kiw_db->db_affected_row > 0) {
            ...
            echo json_encode(array("status" => "error", "message" => "Missing voucher code to process this request", "data" => null));
