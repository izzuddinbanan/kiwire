        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                if(!$is_file_removed && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $metas['file'])){
                    ...
                    if($metas['type'][0] == "image") {


                        $path_webp = $this->options["uploadDir"];
                        $name_webp = pathinfo($metas["name"], PATHINFO_FILENAME) . ".webp";
                        if(!file_exists($path_webp)) mkdir($path_webp, 750, true);
                        $this->convertImageToWebP($metas['file'], "{$path_webp}/{$name_webp}");

                        $metas['name_webp'] = pathinfo($metas["name"], PATHINFO_FILENAME) . ".webp";


                        if(filesize($this->options["uploadDir"].$name_webp) > 0){

                            $path_ori = "{$this->options["uploadDir"]}/original-image";
                            $name_ori = pathinfo($metas["name"], PATHINFO_FILENAME) . ".png";
                            
                            if(!file_exists($path_ori)) mkdir($path_ori, 750, true);
                            
                            rename($metas['file'], "{$path_ori}/{$name_ori}");
