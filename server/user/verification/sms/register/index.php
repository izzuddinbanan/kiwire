...
require_once dirname(__FILE__, 4) . "/includes/include_registration.php";
...
if ($_SESSION['system']['checked'] == true) {
    ...
    if ((!isset($_REQUEST['code']) || empty($_REQUEST['code'])) && ($_REQUEST['method'] == "sms")) {
        ...
        // concatenate mobile code with mobile number

        $kiw_user['ph_code']        = $kiw_db->escape($_REQUEST['mobilecode']);
        $kiw_user['ph_number']      = $kiw_db->escape($_REQUEST['mobilenumber']);

        $kiw_user['phone_number']   = $kiw_user['ph_code'] . $kiw_user['ph_number'];



        // create a random code for the future use. ie otp or password

        $kiw_user['code'] = substr(preg_replace('/\D/', '', md5(time() . $kiw_user['username'])), 6, 6);


        // if use otp pin as password

        if ($_REQUEST['otp-password'] == 'true') {

            $_REQUEST['password']      = $kiw_user['code'];
            $_REQUEST['verification']  = $kiw_user['code'];
        ...
        if (!empty($kiw_signup) && $kiw_signup['enabled'] == "y") {
            ...
            if ($kiw_signup['mode'] == "1") {
                ...
                if (check_account_exist($kiw_db, $kiw_user['username'], $_SESSION['controller']['tenant_id']) == false) {
                    ...
                    $_SESSION['user']['data']       = base64_encode(json_encode($_REQUEST));
                    $_SESSION['user']['otp']        = $kiw_user['code'];
                    ...
                    if (empty($kiw_sms_template)) {
                        ...
                        if (empty($kiw_sms_template)){
                            ...
                            $kiw_sms_template['content'] = "[ WIFI Registration ] Your OTP is {{otp_code}}";
                    ...
                    $kiw_sms['content']     = str_replace("{{otp_code}}", $kiw_user['code'], $kiw_sms['content']);
    ...
    } else if ($_REQUEST['resend'] == "true") {

          // create a random code for the future use. ie otp or password

          $kiw_resend['code'] = substr(preg_replace('/\D/', '', md5(time() . $kiw_user['username'])), 6, 6);


          $kiw_temp = json_decode(base64_decode($_SESSION['user']['data']), true);
        ...
        if (!empty($kiw_temp)) {
            ...
            // resend otp to user for them to verification
  
            $_SESSION['user']['otp'] = $kiw_resend['code'];
            ...
            $kiw_sms['content']      = str_replace("{{tenant_id}}", $_SESSION['controller']['tenant_id'], $kiw_sms_template['content']);
            $kiw_sms['content']      = str_replace("{{username}}", $kiw_temp['username'], $kiw_sms['content']);
            $kiw_sms['content']      = str_replace("{{otp_code}}", $kiw_resend['code'], $kiw_sms['content']);
  
            $kiw_sms['content']      = strip_tags($kiw_sms['content']);
  
            $kiw_sms['action']       = "send_sms";
            $kiw_sms['tenant_id']    = $_SESSION['controller']['tenant_id'];
            $kiw_sms['phone_number'] = $kiw_temp['username'];
            ...
            logger($_SESSION['user']['mac'], "SMS resent to {$kiw_temp['username']}");
  
            unset($kiw_temp);
  
            error_redirect($_SERVER['HTTP_REFERER'], "Your OTP has been resent.");
        ...
        if ($kiw_signup['enabled'] == "y" && $kiw_signup['mode'] == "1") {


            $kiw_user = preg_replace('/\D/', '', $_REQUEST['code']);


            if (!empty($_SESSION['user']['data']) && !empty($_SESSION['user']['otp'])) {


                if ($_SESSION['user']['otp'] == $kiw_user) {
                    ...
                    if (!empty($kiw_temp)) {
                        ...
                        $kiw_temp['phone_number'] = $kiw_temp['mobilecode'] . $kiw_temp['mobilenumber'];


                        if (check_account_exist($kiw_db, $kiw_temp['username'], $_SESSION['controller']['tenant_id']) == false) {


                            $kiw_create_user = array();

                            $kiw_create_user['tenant_id']       = $_SESSION['controller']['tenant_id'];
                            $kiw_create_user['username']        = $kiw_temp['username'];
                            $kiw_create_user['password']        = $kiw_temp['password'];
                            $kiw_create_user['fullname']        = $kiw_temp['fullname'];
                            $kiw_create_user['email_address']   = $kiw_temp['email_address'];
                            $kiw_create_user['phone_number']    = $kiw_temp['phone_number'];
                            ...
                            if (strlen($kiw_signup['data']) > 0 && count($kiw_mapping) > 0) {
                                ...
                                foreach (explode(",", $kiw_signup['data']) as $kiw_data) {
                                    ...
                                    if (isset($kiw_temp[$kiw_data]) && !empty($kiw_temp[$kiw_data])) {
                                        ...
                                        foreach ($kiw_mapping as $kiw_map) {
                                            ...
                                            if ($kiw_map['variable'] == $kiw_data) {
                                                ...
                                                $kiw_create_user[$kiw_map['field']] = $kiw_db->escape($kiw_temp[$kiw_data]);
                                ...
                                $kiw_create_user['phone_number']    = (strlen($kiw_temp['phone_number']) > 0) ? $kiw_db->escape($kiw_temp['phone_number']) : "NA";
                            ...
                            if ($kiw_signup['after_register'] == "internet") {
                                ...
                                // login_user($kiw_temp['username'], $kiw_temp['code'], $session_id);
                                login_user($kiw_temp['username'], $kiw_temp['password'], $session_id);
