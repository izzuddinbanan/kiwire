...
$kiw_temp = $kiw_cache->get("LDAP_DATA:{$_SESSION['controller']['tenant_id']}");

if (empty($kiw_temp)) {


    $kiw_temp = $kiw_db->query_first("SELECT SQL_CACHE * FROM kiwire_int_ldap WHERE tenant_id = '{$_SESSION['controller']['tenant_id']}' LIMIT 1");

    if (empty($kiw_temp)) $kiw_temp = array("dummy" => true);

    $kiw_cache->set("LDAP_DATA:{$_SESSION['controller']['tenant_id']}", $kiw_temp, 1800);
