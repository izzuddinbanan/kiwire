        ...
        if ($kiw_campaign['action'] != "ads"){
                ...
                if ($kiw_user){
                    ...
                    if ($kiw_campaign['action_method'] == "sms"){
                        ...
                        if (!empty($kiw_user['phone_number'])) {
                            ...
                            $kiw_content = stripcslashes($kiw_content['content']);
                            $kiw_content = strip_tags($kiw_content);
            ...
            if ($kiw_ads) {
                ...
                if ($kiw_ads['type'] != "json") {
                    ...
                    if ($kiw_temp && !empty($kiw_ads['json_path'])) {
                        ...
                        if (is_array($kiw_campaign_sorted) && count($kiw_campaign_sorted) > 0) {
                        ...
                        foreach ($kiw_temp as $kiw_json_ads) {
                            ...
                            if (substr($kiw_temp_url, 0, 4) != "http"){
                                ...
                                $kiw_temp_url = $kiw_base_url . ltrim($kiw_temp_url, "/");

                            }


                            $temp_name = $kiw_json_ads[$kiw_mapping['name']];
                            $temp_name = str_replace('\'', "’", $temp_name);


                            $temp_desc = $kiw_json_ads[$kiw_mapping['desc']];
                            $temp_desc = str_replace('\'', "’", $temp_desc);

                            if(!str_contains($kiw_temp_url, '\'')) {


                                $kiw_campaigns_result[$kiw_campaign['c_space']][] = array(
                                    "type"      => "img",
                                    "url"       => urlencode(sync_encrypt($kiw_json_ads[$kiw_mapping['url']])),
                                    "image"     => $kiw_temp_url,
                                    "name"      => "{$kiw_campaign['id']} || " . $temp_name,
                                    "desc"      => $temp_desc,
