...
go(function () {
    ...
    $kiw_cloud['tenant_id'] = "jendela-measat";
    ...
    $kiw_get_profiles = $kiw_db->query("SELECT * FROM kiwire_auto_reset WHERE tenant_id = '{$kiw_cloud['tenant_id']}' AND exec_when = 'cd'");

    //terminate scheduler if auto reset is disable
    if (empty($kiw_get_profiles)) die();



        $profile_names = array_column($kiw_get_profiles, 'profile');

        $kiw_list_profiles = "('" . implode("', '", $profile_names) . "')";


        // $kiw_list_profiles = "('JENDELA-MEASAT-NEW-USER', 'JENDELA-MEASAT-12GB-USER')";
        ...
        foreach ($kiw_profiles as $kiw_profile) {
            ...
            foreach ($kiw_accounts as $kiw_account) {
                ...
                $kiw_date["renew"] = date("Y-m-d 00:00:00", strtotime("+1 Day", strtotime($kiw_account['date_activate'])));
                ...
                if ($kiw_date["renew"] == $kiw_cur_date) {
                    ...
                    if (!empty($kiw_account['profile_cus']) && $kiw_cf['carry_forward_topup'] == 'y') {
                        ...
                        $kiw_db->query("UPDATE kiwire_account_auth SET session_time = 0, quota_in = 0, quota_out = 0, date_activate = '{$kiw_date["renew"]}', profile_curr = profile_subs, profile_cus = '{$kiw_temp}' WHERE tenant_id = '{$kiw_cloud['tenant_id']}' AND profile_subs = '{$kiw_profile['name']}' AND date_activate = '{$kiw_account["date_activate"]}'");
                    
                    } else {


                        $kiw_db->query("UPDATE kiwire_account_auth SET session_time = 0, quota_in = 0, quota_out = 0, date_activate = '{$kiw_date["renew"]}', profile_curr = profile_subs, profile_cus = '' WHERE tenant_id = '{$kiw_cloud['tenant_id']}' AND profile_subs = '{$kiw_profile['name']}' AND date_activate = '{$kiw_account["date_activate"]}'");
