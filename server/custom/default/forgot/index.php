...
if ($_SESSION['system']['checked'] == "true") {
    ...
    if (is_array($kiw_user) && !empty($kiw_user)){
        ...
        $kiw_user_details = $kiw_db->query_first("SELECT username,phone_number FROM kiwire_account_auth WHERE username = '{$kiw_username}' AND tenant_id = '{$_SESSION['controller']['tenant_id']}' LIMIT 1");
        ...
        if ($kiw_cloud_config['forgot_password_method'] == "email"){
            ...
            if ($kiw_username == $kiw_user_details['username']){


                $kiw_db->query("UPDATE kiwire_account_auth SET password = '{$kiw_enc_password}' WHERE tenant_id = '{$_SESSION['controller']['tenant_id']}' AND username = '{$kiw_username}' LIMIT 1");
                ...
                if (empty($kiw_email_template)) {
                    ...
                    if (empty($kiw_email_template)){
                        ...
                        $kiw_email_template['content'] = @file_get_contents(dirname(__FILE__, 4) . "/user/templates/user-forgot-password.html");
