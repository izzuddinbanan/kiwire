...
function pull_data() {
    ...
    $.ajax({
        url: "ajax/ajax_report_login_logins_concurrency.php",
        method: "POST",
        data: {
            ...
            if (data['status'] === "success") {
                ...
                if (data['data'].length > 0) {
                    ...
                    for (let x = 0; x < data['data'].length; x++) {
                        ...
                        table_str += "<td>" + data['data'][x]['concurrent'] + "</td>";
                        table_str += "<td><a href='#' data-report-date='" + data['data'][x]['xreport_date'] + "' class='btn btn-primary btn-sm fa fa-search btn-report-details'></a></td>";
                        table_str += "</tr>";

                        chart_concurrent.push([(data['data'][x]['xreport_date']), (data['data'][x]['concurrent'])]);
...
function get_report_per_date(report_date) {
    ...
    if (report_date.length > 0) {
        ...
        $.ajax({
            url: "/admin/ajax/ajax_report_login_logins_concurrency.php",
            method: "get",
            data: {
            ...
            success: function (response) {
                ...
                if (response['status'] === "success") {
                    ...
                    for (let x = 0; x < response['data'].length; x++) {
                        ...
                        table_str += "<td>" + response['data'][x]['concurrent'] + "</td>";
                        table_str += "</tr>";

                        chart_date.push(response['data'][x]['xreport_date']);
                        chart_concurrent.push(response['data'][x]['concurrent']);
