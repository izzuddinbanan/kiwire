...
function pull_data() {
    ...
    $.ajax({
        url: "ajax/ajax_report_login_logins_device_freq.php",
        method: "POST",
        data: {
            ...
            if (data['status'] === "success") {
                ...
                if (data['data'].length > 0) {
                    ...
                    for (let x = 0; x < data['data'].length; x++) {
                        ...
                        table_str += "<td>" + data['data'][x]['device_unique'] + "</td>";
                        table_str += "<td>" + data['data'][x]['device_new'] + "</td>";
                        ...
                        chart_total.push([data['data'][x]['xreport_date'], parseInt(data['data'][x]['total'])]);
                        chart_unique.push([data['data'][x]['xreport_date'], parseInt(data['data'][x]['device_unique'])]);
                        chart_new.push([data['data'][x]['xreport_date'], parseInt(data['data'][x]['device_new'])]);
...
function get_report_per_date(report_date) {
    ...
    if (report_date.length > 0) {
        ...
        $.ajax({
            url: "/admin/ajax/ajax_report_login_logins_device_freq.php",
            method: "get",
            data: {
            ...
            success: function (response) {
                ...
                if (response['status'] === "success") {
                    ...
                    for (let x = 0; x < response['data'].length; x++) {
                        ...
                        table_str += "<td>" + response['data'][x]['total'] + "</td>";
                        table_str += "<td>" + response['data'][x]['device_unique'] + "</td>";
                        table_str += "<td>" + response['data'][x]['device_new'] + "</td>";
                        table_str += "</tr>";

                        chart_date.push(response['data'][x]['xreport_date']);
                        chart_total.push(parseInt(response['data'][x]['total']));
                        chart_unique.push(parseInt(response['data'][x]['device_unique']));
                        chart_new.push(parseInt(response['data'][x]['device_new']));
