...
function pull_data() {
    ...
    $.ajax({
        url: "ajax/ajax_report_login_logins_freq.php",
        method: "POST",
        data: {
        ...
        success: function (data) {
            ...
            if (data['status'] === "success") {
                ...
                if (data['data'].length > 0) {
                    ...
                    let chart_total = [];
                    let chart_unique_daily = [];
                    let chart_unique_forever = [];
                    ...
                    for (let x = 0; x < data['data'].length; x++) {
                        ...
                        table_str += "<td>" + data['data'][x]['login_total'] + "</td>";
                        table_str += "<td>" + data['data'][x]['login_unique'] + "</td>";
                        table_str += "<td>" + data['data'][x]['login_new'] + "</td>";
                        ...
                        chart_total.push([data['data'][x]['xreport_date'], parseInt(data['data'][x]['login_total']).toFixed(0)]);
                        chart_unique_daily.push([data['data'][x]['xreport_date'], parseInt(data['data'][x]['login_unique']).toFixed(0)]);
                        chart_unique_forever.push([data['data'][x]['xreport_date'], parseInt(data['data'][x]['login_new']).toFixed(0)]);
                    ...
                    var report_chart_option = {
                        ...
                        series: [{
                            ...
                            name: "Unique Daily",
                            data: chart_unique_daily
                        }, {
                            name: "Unique Forever",
                            data: chart_unique_forever
                        }]
                    };
...
function get_report_per_date(report_date) {
    ...
    if (report_date.length > 0) {
        ...
        $.ajax({
            url: "/admin/ajax/ajax_report_login_logins_freq.php",
            method: "get",
            data: {
            ...
            success: function (response) {
                ...
                if (response['status'] === "success") {
                    ...
                    for (let x = 0; x < response['data'].length; x++) {
                        ...
                        table_str += "<td>" + response['data'][x]['login_total'] + "</td>";
                        table_str += "<td>" + response['data'][x]['login_unique'] + "</td>";
                        table_str += "<td>" + response['data'][x]['login_new'] + "</td>";
                        table_str += "</tr>";

                        chart_date.push(response['data'][x]['xreport_date']);
                        chart_total.push(response['data'][x]['login_total']);
                        chart_unique_daily.push(response['data'][x]['login_unique']);
                        chart_unique_forever.push(response['data'][x]['login_new']);
