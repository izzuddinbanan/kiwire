...
function pull_data() {
    ...
    $.ajax({
        url: "ajax/ajax_report_account_voucher_availability.php?action=get_by_date",
        method: "POST",
        data: {
            ...
            if (data['status'] === "success") {
                ...
                if (data['data'].length > 0) {
                    ...
                    for (let x = 0; x < data['data'].length; x++) {
                        ...
                        table_str += "<td><a href=\"javascript:void(0);\" onclick=\"freshPrepaid('" + data['data'][x]['bulk_id'] + "')\" class='btn btn-icon btn-warning btn-xs mr-1 fa fa-search'></a></td>";
                    ...
                    table_str += '<tr><td colspan="9" align="center">No data available in table</td></tr>';
...
function freshPrepaid(bulk_id) {

    if (bulk_id.length) {

        $.ajax({
            url: "ajax/ajax_report_account_voucher_availability.php?action=view_activatedPrepaid",
            method: "POST",
            data: {
                "bulk_id": bulk_id,
                "startdate": $('#startdate').val(),
            ...
            success: function (data) {
                ...
                if (data['status'] === "success") {
                    ...
                    if (data['data'].length > 0) {
                        ...
                        table_str_1 = "<thead><tr class='text-uppercase'><th>No</th><th>Username</th><th>Create Date</th><th>Expiry Date</th><th>Unit Price(MYR)</th></tr></thead>";
                        ...
                        for (let x = 0; x < data['data'].length; x++) {
                            ...
                            table_str_1 += "<td>" + data['data'][x]['username'] + "</td>";
        ...
        $.ajax({
            url: "ajax/ajax_report_account_voucher_availability.php?action=view_freshPrepaid",
            method: "POST",
            data: {
                "bulk_id": bulk_id,
                "startdate": $('#startdate').val(),
            ...
            success: function (data) {
                ...
                if (data['status'] === "success") {
                    ...
                    if (data['data'].length > 0) {
                        ...
                        table_str_2 = "<thead><tr class='text-uppercase'><th>No</th><th>Username</th><th>Create Date</th><th>Expiry Date</th><th>Unit Price(MYR)</th></tr></thead>";
                        ...
                        for (let x = 0; x < data['data'].length; x++) {
                            ...
                            table_str_2 += "<td>" + data['data'][x]['username'] + "</td>";
