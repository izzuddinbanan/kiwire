...
function create()
{
    ...
    if (in_array($_SESSION['permission'], array("w", "rw"))) {
        ...
        $kiw_row = $kiw_db->query_first("SELECT COUNT(*) AS ccount FROM kiwire_int_api_setting WHERE api_key = '" . $kiw_db->escape($_GET['api_key']) . "' AND tenant_id = '$tenant_id'");
...
function delete()
{
    ...
    if (in_array($_SESSION['permission'], array("w", "rw"))) {
        ...
        if (!empty($id)) {
            ...
            $kiw_temp = $kiw_db->query_first("SELECT * FROM kiwire_int_api_setting WHERE id = '" . $id . "' AND tenant_id = '$tenant_id'");

            $sql = $kiw_db->query("DELETE FROM kiwire_int_api_setting WHERE id = '" . $id . "' AND tenant_id = '$tenant_id'");
        }


        sync_logger("{$_SESSION['user_name']} deleted Authentication Key {$kiw_temp['api_key']}", $_SESSION['tenant_id']);
...
function get_data()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        $kiw_temp = $kiw_db->fetch_array("SELECT  * FROM kiwire_int_api_setting WHERE tenant_id = '{$tenant_id}' LIMIT 1000");
...
function get_single_data()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        $kiw_temp = $kiw_db->query_first("SELECT * FROM kiwire_int_api_setting  WHERE tenant_id = '$tenant_id' AND id = '{$id}' LIMIT 1");
...
function edit_single_data()
{
    ...
    if (in_array($_SESSION['permission'], array("w", "rw"))) {
        ...
        $is_24_hour     = isset($_REQUEST['is_24']) && $_REQUEST['is_24'] ? 1 : 0;
        $start_time     = $kiw_db->escape($_REQUEST['start_time']);

        if($_REQUEST['start_time'] == $_REQUEST['stop_time'])
            die(json_encode(array("status" => "failed", "message" => "ERROR: Stop Time cannot same as Start Time", "data" => null)));
        else
            $stop_time =   $kiw_db->escape($_REQUEST['stop_time']);


        $kiw_db->query("UPDATE kiwire_int_api_setting SET updated_date = NOW(), enabled = '{$new_enabled}', api_key = '{$new_apikey}',  permission = '{$new_permission}', module = '{$new_groupname}', is_24_hour = '{$is_24_hour}', start_time = '{$start_time}', stop_time = '{$stop_time}'  WHERE id = '{$id}' AND tenant_id = '{$tenant_id}' LIMIT 1");


        sync_logger("{$_SESSION['user_name']} updated Authentication Key {$new_apikey}", $_SESSION['tenant_id']);
