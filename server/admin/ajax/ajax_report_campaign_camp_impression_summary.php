...
switch ($action) {
    ...
    case "get_impress_summary":
        get_impress_summary($kiw_db, $_SESSION['tenant_id']);
        break;
    default:
...
function get_by_date($kiw_db, $kiw_tenant)
{
    ...
    $kiw_results = $kiw_db->fetch_array("SELECT name, SUM(IFNULL(impress, 0)) AS impress, SUM(IFNULL(u_impress, 0)) AS u_impress,source FROM kiwire_report_campaign_general WHERE tenant_id = '{$kiw_tenant}' AND (report_date BETWEEN '{$kiw_date_start}' AND '{$kiw_date_end}') {$kiw_zone} {$kiw_project} GROUP BY name");
    ...
    foreach ($kiw_results as $kiw_result) {
        ...
        $kiw_now = array_merge($kiw_campaign_list[trim($kiw_id[0])], array("name" => $kiw_id[1], "impress" => $kiw_result['impress'], "u_impress" => $kiw_result['u_impress'], "source" => ucfirst($kiw_result['source']), "reference" => $kiw_result['name']));
...
function get_impress_summary($kiw_db, $kiw_tenant)
{
    ...
    $kiw_results = $kiw_db->fetch_array("SELECT CONVERT_TZ(report_date, 'UTC', '{$kiw_timezone}') AS xreport_date, SUM(IFNULL(impress, 0)) AS impress, SUM(IFNULL(u_impress, 0)) AS u_impress FROM kiwire_report_campaign_general WHERE tenant_id = '{$kiw_tenant}' AND (report_date BETWEEN '{$kiw_date_start}' AND '{$kiw_date_end}') AND name = '{$kiw_campaign_name}' {$kiw_zone} {$kiw_project} GROUP BY xreport_date ORDER BY report_date ASC");
    ...
    foreach ($kiw_results as $kiw_result) {
        ...
        $kiw_result_list['impress'][] = $kiw_result['impress'];
        $kiw_result_list['u_impress'][] = $kiw_result['u_impress'];
    }
