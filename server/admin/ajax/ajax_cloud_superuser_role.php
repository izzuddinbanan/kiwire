...
function get_all()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        $kiw_temp = $kiw_db->fetch_array("SELECT DISTINCT(groupname) AS groupname FROM kiwire_admin_group WHERE tenant_id = 'superuser' AND groupname <> '' ORDER BY groupname ASC");
    ...
    if (in_array($_SESSION['permission'], array("w", "rw"))) {
        ...
        if (empty($kiw_update)) {
            ...
            if (!empty($kiw_groupname)) {
                ...
                $kiw_temp = $kiw_db->query_first("SELECT COUNT(*) AS kcount FROM kiwire_admin_group WHERE groupname = '{$kiw_groupname}' AND tenant_id = 'superuser'");
                ...
                if ($kiw_temp['kcount'] == 0) {
                    ...
                    if (count($kiw_role_list) > 0) {
                        ...
                        foreach ($kiw_role_list as $kiw_role) {
                            ...
                            $kiw_db->query("INSERT INTO kiwire_admin_group VALUE (NULL, '{$kiw_groupname}', '" . $kiw_db->escape($kiw_role) . "', 'superuser', NOW())");
            ...
            if (!empty($kiw_update)) {
                ...
                if (count($kiw_role_list) > 0) {
                    ...
                    $kiw_db->query("DELETE FROM kiwire_admin_group WHERE groupname = '{$kiw_update}' AND tenant_id = 'superuser'");


                    foreach ($kiw_role_list as $kiw_role) {

                        $kiw_db->query("INSERT INTO kiwire_admin_group VALUE (NULL, '{$kiw_update}', '" . $kiw_db->escape($kiw_role) . "', 'superuser', NOW())");

                    }


                    sync_logger("{$_SESSION['user_name']} updated role {$kiw_update}", $_SESSION['tenant_id']);

                    echo json_encode(array("status" => "success", "message" => "SUCCESS: Role [{$kiw_update}] has been updated", "data" => null));
...
function delete()
{
    ...
    if (in_array($_SESSION['permission'], array("w", "rw"))) {
        ...
        if (!empty($kiw_group_name)) {
            ...
            $kiw_db->query("DELETE FROM kiwire_admin_group WHERE groupname = '{$kiw_group_name}' AND tenant_id = 'superuser'");
...
function get_update()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        $kiw_temp = $kiw_db->fetch_array("SELECT * FROM kiwire_admin_group WHERE tenant_id = 'superuser' AND groupname = '{$kiw_group_name}'");
