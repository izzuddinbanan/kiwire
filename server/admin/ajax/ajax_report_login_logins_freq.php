...
function get_all()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        // $kiw_temp = $kiw_db->fetch_array("SELECT DATE(CONVERT_TZ(report_date, 'UTC', '{$timezone}')) AS xreport_date, SUM(succeed) AS login_total, SUM(account_new) AS login_new, SUM(account_unique) AS login_unique FROM kiwire_report_login_general WHERE tenant_id = '{$_SESSION['tenant_id']}' AND (report_date BETWEEN '{$startdate}' AND '{$enddate}') GROUP BY xreport_date");
