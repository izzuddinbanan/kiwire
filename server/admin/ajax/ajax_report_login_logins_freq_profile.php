...
function get_all()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        // $kiw_temp = $kiw_db->fetch_array("SELECT DATE(CONVERT_TZ(report_date, 'UTC', '{$timezone}')) AS xreport_date, profile, SUM(login) AS login, SUM(u_login) AS ulogin FROM kiwire_report_login_profile WHERE tenant_id = '{$_SESSION['tenant_id']}' AND (report_date BETWEEN '{$startdate}' AND '{$enddate}') {$profile} GROUP BY xreport_date,profile");

        $kiw_temp = $kiw_db->fetch_array("SELECT DATE(CONVERT_TZ(report_date, 'UTC', '{$timezone}')) AS xreport_date, profile, SUM(login) AS login, SUM(u_login) AS ulogin FROM kiwire_report_login_profile WHERE tenant_id = '{$_SESSION['tenant_id']}' AND (report_date BETWEEN '{$startdate}' AND '{$enddate}') {$profile} {$kiw_zone} {$kiw_project} GROUP BY xreport_date,profile");

        echo json_encode(array("status" => "success", "message" => "", "data" => $kiw_temp));
    }
}
...
function get_detail()
{
    ...
    $kiw_start = sync_toutctime(date("Y-m-d H:i:s", strtotime($_REQUEST['report_date'])), $timezone);

    $kiw_end = date("Y-m-d H:i:s", strtotime($kiw_start . " +1 Day -1 Second"));
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        $kiw_temp = $kiw_db->fetch_array("SELECT CONVERT_TZ(report_date, 'UTC', '{$timezone}') AS xreport_date, profile, SUM(login) AS login, SUM(u_login) AS ulogin FROM kiwire_report_login_profile WHERE tenant_id = '{$_SESSION['tenant_id']}' AND (report_date BETWEEN '{$kiw_start}' AND '{$kiw_end}') {$profile} {$kiw_zone} {$kiw_project} GROUP BY xreport_date,profile");
