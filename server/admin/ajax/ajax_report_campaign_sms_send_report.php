...
function calculate_totalAvg()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        $kiw_temp = $kiw_db->query_first("SELECT SUM(sms) AS total FROM kiwire_report_login_general WHERE tenant_id = '{$_SESSION['tenant_id']}' AND (report_date BETWEEN '{$startdate}' AND '{$enddate}') {$kiw_zone} {$kiw_project}");
...
function get_all()
{
    ...
    if (in_array($_SESSION['permission'], array("r", "rw"))) {
        ...
        $kiw_temp = $kiw_db->fetch_array("SELECT DATE(CONVERT_TZ(report_date, 'UTC', '{$timezone}')) AS xreport_date, SUM(sms) AS total FROM kiwire_report_login_general WHERE tenant_id = '{$_SESSION['tenant_id']}' AND (report_date BETWEEN '{$startdate}' AND '{$enddate}') {$kiw_zone} {$kiw_project} GROUP BY xreport_date");
