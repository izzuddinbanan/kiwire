...
foreach ($kiw_clouds as $kiw_cloud) {
    ...
    $kiw_accounts = $kiw_db->fetch_array("SELECT username FROM kiwire_account_auth WHERE tenant_id = '{$kiw_cloud}' AND status = 'suspend' AND date_create < DATE_SUB(NOW(), INTERVAL 10 MINUTE)");
