...
$kiwire_server->on("request", function ($request, $response) {
    ...
    if (empty($kiw_dupe_check)) {
        ...
        if (empty($kiw_request['tenant_id'])) {
            ...
            $kiw_temp = $kiw_cache->get("NAS_TENANT:{$kiw_id_hash}");


            if (empty($kiw_temp)) {


                $kiw_temp = $kiw_db->query("SELECT SQL_CACHE tenant_id FROM kiwire_controller WHERE unique_id = '{$kiw_request['nasid']}' LIMIT 1")[0];

                if (empty($kiw_temp)) $kiw_temp['tenant_id'] = "XXinvalidXX";
                ...
                $kiw_request['tenant_id'] = $kiw_temp;
        ...
        if ($kiw_request['tenant_id'] != "XXinvalidXX") {
            ...
            if (empty($kiw_request['zone'])){
                ...
                if (empty($kiw_zone_attr)) {
                    ...
                    $kiw_zone_attr = $kiw_db->query("SELECT SQL_CACHE * FROM kiwire_zone_child WHERE tenant_id = '{$kiw_request['tenant_id']}' ORDER BY priority DESC");
                ...
                if (in_array($kiw_request['tenant_id'], array("GBTB_UAT", "GBTB"))){
                    ...
                    $kiw_temp = "GBTB_ZONE_DATA:" . str_replace(array(":", "-"), "", strtoupper($kiw_request['mac_address']));


                    $kiw_gbtb = $kiw_cache->get($kiw_temp);
            ...
            if ($kiw_request['action'] == "authorize") {
                ...
                if ($kiw_request['type'] != "start") {
                    ...
                    if (is_array($kiw_session)) {
                        ...
                        if (!empty($kiw_request['ip_address']) && substr($kiw_current_session['ip_address'], 0, 4) == "169."){
                        ...
                        $kiw_db->query(sql_update($kiw_db, $kiw_session['session_table'], $kiw_current_session, "unique_id = '{$kiw_session['unique_id']}' AND tenant_id = '{$kiw_session['tenant_id']}' LIMIT 1"));
                    ...
                    if ($kiw_disconnect == true){
                        ...
                        if (strlen($kiw_profile['advance']) > 0){
                            ...
                            // do disconnect since no more advance profile
                ...
                check_logger("T:{$kiw_request['tenant_id']} N:{$kiw_request['nasid']} M:{$kiw_request['mac_address']} = Accounting Received and Responded [ " . ucfirst($kiw_request['type']) . " ]", $kiw_request['tenant_id']);
