...
$kiw_temp = $kiw_cache->get("RADIUS_DATA:{$kiw_tenant_id}");

if (empty($kiw_temp)) {


    $kiw_temp = $kiw_db->query_first("SELECT SQL_CACHE * FROM kiwire_int_radius WHERE enabled = 'y' AND tenant_id = '{$kiw_tenant_id}' LIMIT 1");

    if (empty($kiw_temp)) $kiw_temp = array("dummy" => true);

    $kiw_cache->set("MSAD_DATA:{$kiw_tenant_id}", $kiw_temp, 1800);
...
if ($kiw_temp['enabled'] == "y"){
    ...
    $kiw_notification = $kiw_cache->get("NOTIFICATION_DATA:{$kiw_tenant_id}");

    if (empty($kiw_notification)) {


        $kiw_notification = $kiw_db->query_first("SELECT SQL_CACHE * FROM kiwire_notification WHERE tenant_id = '{$kiw_tenant_id}' LIMIT 1");

        if (empty($kiw_notification)) $kiw_notification = array("dummy" => true);

        $kiw_cache->set("NOTIFICATION_DATA:{$kiw_tenant_id}", $kiw_notification, 1800);
    ...
    $kiw_data['tenant_id']  = $kiw_tenant_id;
    ...
    if ($kiw_login_status['status'] == "success"){
        ...
        $_SESSION['controller']['tenant_id'] = $kiw_tenant_id;
        ...
        header("Location: /user/pages/public/?page=35aa98ff&tenant=stormwifi&session={$_GET['session']}");
