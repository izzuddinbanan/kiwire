...
$kiw_datas = fopen(dirname(__FILE__) . "/impress.log", "r");
...
while (!feof($kiw_datas)){
    ...
    if ($kiw_campaign[2] > $kiw_start_time && $kiw_campaign[2] < $kiw_stop_time) {
        ...
        if (!empty($kiw_campaign[3])) {
            ...
            $kiw_db->query("UPDATE kiwire_report_campaign_general SET updated_date = NOW(), impress = impress + 1 WHERE name = '{$kiw_campaign[1]}' AND tenant_id = '{$kiw_campaign[3]}' AND report_date = '{$kiw_campaign[0]}' LIMIT 1");

            if ($kiw_db->affected_rows < 1) {

                $kiw_db->query("INSERT INTO kiwire_report_campaign_general (id, report_date, tenant_id, zone, updated_date, name, source, impress, click, u_impress, u_click) VALUE (NULL, '{$kiw_campaign[0]}', '{$kiw_campaign[3]}', 'AEM', NOW(), '{$kiw_campaign[1]}', 'external', 1, 0, 0, 0)");
